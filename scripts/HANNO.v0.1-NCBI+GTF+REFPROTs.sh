...
##VARIABLES: $1=genome.fasta, $2=workingdir, $3=protein.faa (from NCBI refSeq), $4=_rna_from_genomic.fna (from NCBI refSeq) $5=BUSCO-LINEAGE-DIR $6=STRINGTIE.gtf(from same genome!) $7=REFPROTDB
...
ls prot-denovo.stringtie-like.gtf $6 > $3.gtf.list2
...
run_BUSCO.py -i Merged.CDS2.faa -o Merged.CDS2.BUSCO -l $5 -m proteins -c $THREADS 
...
grep ">" $7 | awk '{l=length($1);print substr($0,2,l-1)"\t"substr($0,l+2)}' > PROT-DB.desc.txt
...
run_BUSCO.py -i Merged.CDS2.gtf.clustered.cds.faa -o Merged.CDS3.BUSCO -l $5 -m proteins -c $THREADS 
#rm tmp run_Merged.CDS* -rf
