...
i=0;j=1;frame=0;frameout=0;cdslen=0;cstart=0;cdscount=0;score=0;
...
j=0;
#constant score yields slightly better results
score=100;

if($3=="+" || $3=="."){
print $2"\tStringTie\ttranscript\t"$4+1"\t"$5"\t"score"\t"$3"\t.\tgene_id \""$1"\"; transcript_id \""$1"\"; cov \""score/10"\"; FPKM \""score/5"\"; TPM \""score/2"\";";
for(x=1;x<n3;x=x+2) 	{
			print $2"\tStringTie\texon\t"allE[x]+1"\t"allE[x+1]"\t"score"\t"$3"\t.\tgene_id \""$1"\"; transcript_id \""$1"\";"
			if(x>=cstart && x<=cstart+i)	{
							cdscount++;
...
#no need for CDS in TACO
#									print $2"\tStringTie\tCDS\t"allC[j]+1"\t"allC[j+1]"\t"score"\t"$3"\t"frameout"\tgene_id \""$1"\"; transcript_id \""$1"\";";
							j=j+2
							}
			};
}
...
#output sorted by coordinate for TACO!
j=0;
print $2"\tStringTie\ttranscript\t"$4+1"\t"$5"\t"score"\t"$3"\t.\tgene_id \""$1"\"; transcript_id \""$1"\"; cov \""score/10"\"; FPKM \""score/5"\"; TPM \""score/2"\";";
for(x=1;x<n3;x=x+2)     {
                        print $2"\tStringTie\texon\t"allE[x]+1"\t"allE[x+1]"\t"score"\t"$3"\t.\tgene_id \""$1"\"; transcript_id \""$1"\";"
