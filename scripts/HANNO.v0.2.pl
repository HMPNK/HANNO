...
##GET OPTIONS
my %options=();
getopts("a:d:p:P:m:b:g:t:r:", \%options);
...
sed \"s/MSTRG./MSTRG/g\" MODELS2.gtf > MODELS2.gtf.t 
...
bedtools getfasta -split -nameOnly -s -bed MODELS2.CDS2only.bed12 -fi asm.fa -fo /dev/stdout | $scr/TRANSLATE.sh > MODELS2.CDS2.faa
##CLEAN-UP
...
awk -v eggnog=out.emapper.annotations -v last=MODELS2.CDS2_vs_PROTDB.description.txt \'BEGIN{OFS=\"\\t\";FS=\"\\t\";while(getline l < eggnog){n=split(l,a,\"\\t\");if(substr(l,1,1)!=\"#\"){split(a[1],b,\".\");i++;nlist[i]=b[1];t[b[1]]=1;egg3[b[1]]=a[3];egg4[b[1]]=a[4];egg8[b[1]]=a[8];egg9[b[1]]=a[9];;egg11[b[1]]=a[11];}};while(getline l < last){n=split(l,a,\"\\t\");if(substr(l,1,1)!=\"#\"){split(a[1],b,\".\");if(t[b[1]]==\"\"){i++;nlist[i]=b[1];};last2[b[1]]=a[2];last3[b[1]]=a[3]}};exit} END{for(x=1;x<=i;x++){y=nlist[x];\$0=y\"\\t\"egg9[y]\"\\t\"egg8[y]\"\\t\"egg11[y]\"\\t\"egg3[y]\"\\t\"egg4[y]\"\\t\"last2[y]\"\\t\"last3[y];print}}' | sort -k1,1V > final_description.txt
";
