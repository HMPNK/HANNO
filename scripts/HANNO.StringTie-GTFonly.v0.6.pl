...
if(! $ARGV[0] && ! $options{a})       {
       print STDERR "
HANNO version 0.6 STRINGTIE GTF-ONLY VERSION (High-throughput ANNOtation for eukaryote genomes)
...
bedtools getfasta -name -split -s -bed MODELS2.bed12 -fi asm.fa -fo /dev/stdout | cut -f 1 -d '(' > MODELS2.mrna.fa
...
#correct for some issues in tandem duplicated gene clusters (bedtools intersect -sorted is a must here(Otherwise bedtools crashes)! Introns must be sorted! sort -k1,1V -k2,2n )
awk -f $scr/bed12intronsbed6.awk ALLMODELS-FINAL.bedDB | sort -k1,1V -k2,2n > ALLMODELS-FINAL.introns.bed
bedtools intersect -sorted -F 0.01 -s -split -wao -b ALLMODELS-FINAL.bedDB -a ALLMODELS-FINAL.introns.bed 2>/dev/null | cut -f 1-6,7-11 | awk -f $scr/extract-tandemDup.issues.awk | cut -f 1 -d '_' | sort -Vu > tandemdup-issues.list
echo;echo \"transcript models with issues related to tandem gene duplications (will be removed):\"
wc -l tandemdup-issues.list
#reset and filter ALLMODELS-FINAL.bedDB
grep -vwFf tandemdup-issues.list ALLMODELS-FINAL.bedDB | awk \'BEGIN{OFS=\"\\t\";FS=OFS} {\$4=\$13;print}\' | cut -f 1-12 | grep -v '^#' > ALLMODELS.bed12
head -1 ALLMODELS-FINAL.bedDB > tandemdup-issues.bedDB
grep -wFf tandemdup-issues.list ALLMODELS-FINAL.bedDB | awk \'BEGIN{OFS=\"\\t\";FS=OFS} {\$4=\$13;print}\' >> tandemdup-issues.bedDB
$scr/CLUSTERBYCDS.sh ALLMODELS.bed12 0.1
#re-build database
bash $scr/CREATE-DB.sh ALLMODELS.bed12 ALLMODELS.bed12.model_clusters.tsv ALLMODELS.lastp.description.txt ALLMODELS.eggnog.description.txt ALLMODELS.BUSCO.tsv > ALLMODELS-FINAL.bedDB
#clean-up
rm -f ALLMODELS.bed12 ALLMODELS.bed12.model_clusters.tsv ALLMODELS.cds.fa ALLMODELS.eggnog.description.txt ALLMODELS.faa ALLMODELS.lastp.description.txt ALLMODELS.mRNA.fa ALLMODELS.BUSCO.tsv orf.length ALLMODELS-FINAL.introns.bed tandemdup-issues.list

#select BESTMODELS from ALLMODELS
